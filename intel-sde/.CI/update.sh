...
_update() {
  local _response _url _sha _v1 _v2 _regex
  _response=$(curl -s "https://www.intel.com/content/www/us/en/download/684897/intel-software-development-emulator.html")
  _response=$(sed 's/</\n</g;s/>/>\n/g' <<< "$_response")
  ...
  if curl -Issf "$_url" > /dev/null && [ -n "$_v1" ] && [ -n "$_v2" ]; then
