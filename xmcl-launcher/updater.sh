...
if [ ! -f "$amd64_sha256_file" ]; then
    ...
    wget -O "$amd64_sha256_file" "https://github.com/Voxelum/x-minecraft-launcher/releases/download/$latest_version/xmcl-${latest_version#v}-amd64.deb.sha256"
fi

if [ ! -f "$arm64_sha256_file" ]; then
    color_echo "36" "Downloading SHA256 for arm64..."
    wget -O "$arm64_sha256_file" "https://github.com/Voxelum/x-minecraft-launcher/releases/download/$latest_version/xmcl-${latest_version#v}-arm64.deb.sha256"
fi
